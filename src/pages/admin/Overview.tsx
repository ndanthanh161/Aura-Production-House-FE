...
const AdminOverview: React.FC = () => {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <header>
                <h1 style={{ fontSize: '2rem', color: '#fff' }}>Dashboard Overview</h1>
                <p style={{ color: 'rgba(255,255,255,0.5)' }}>Manage your business performance and metrics.</p>
            </header>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                {[
                    ...
                    { label: 'Total Orders', value: '124', icon: <ShoppingCart size={20} />, trend: '+8.2%' },
                    { label: 'Total Customers', value: '89', icon: <Users size={20} />, trend: '+5.4%' },
                    { label: 'Active Projects', value: '18', icon: <Briefcase size={20} />, trend: '+2' },
                ].map((stat, i) => (
                    <motion.div
                        key={stat.label}
                        ...
                        style={{ backgroundColor: '#0a0a0a', padding: '1.5rem', border: '1px solid #1a1a1a' }}
                    >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem' }}>
                <div style={{ backgroundColor: '#0a0a0a', padding: '1.5rem', border: '1px solid #1a1a1a', minHeight: '400px' }}>
                    <h3 style={{ fontSize: '1.125rem', color: '#fff', marginBottom: '2rem' }}>Revenue Analytics</h3>
                ...
                <div style={{ backgroundColor: '#0a0a0a', padding: '1.5rem', border: '1px solid #1a1a1a' }}>
                    <h3 style={{ fontSize: '1.125rem', color: '#fff', marginBottom: '2rem' }}>Service Breakdown</h3>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem' }}>
                        {pieData.map((d, i) => (
                            <div key={d.name} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                                ...
                                <span style={{ color: '#fff' }}>{d.value} sales</span>
                            </div>
                        ))}
                    </div>
                </div>
